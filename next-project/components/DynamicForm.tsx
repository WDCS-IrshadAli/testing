...
import { createForm } from '@/app/lib/action'
import Link from 'next/link'



const DynamicForm = ({ formData, params}: any) => {  
      const router  = useRouter();
      
      // dynamic form elements handling
      let data = formData?.data[0]?.schema;
      let title = formData?.data[0]?.name;
      let formId = formData?.data[0]?.id;
                    ...
                    <div key={index} className={`w-full flex flex-col gap-1`}>
                      ...
                      {/* DESCRIPTION FIELD */}
                      {
                        ...
                        <p className="text-xs italic font-normal text-gray-100 flex">
                          <InfoCircledIcon className="mr-1" />{fieldData?.help}
                        </p>
                        : ""
                      }
                    </div>
      ...
      const createFormWithId = createForm.bind(null, formId);
        ...
        <>
          <div className="py-3 sm:py-6 pb-10 relative">
              ...
              <h1 className="text-3xl font-semibold px-3 sm:px-6 mb-10">
                <span className="text-sm pe-3"><Link href="/"><Button variant={'outline'}><ChevronLeftIcon className="h-4 w-4" /></Button></Link></span>
